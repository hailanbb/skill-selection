...
from sources import canonical_note, classify
...
def existing_notes(note_dir: Path) -> dict[str, Path]:
    ...
    for path in sorted(note_dir.rglob("*.md")):
        ...
        if source["kind"] == "note":
            result.setdefault(source["note_id"], path)
    return result
...
def validate_evidence(meta: dict[str, Any], allow_partial: bool) -> tuple[dict[str, str], list[str]]:
    source = canonical_note(meta.get("source_url", ""))
...
def publish(manifest: Path, draft: Path, run_dir: Path, *, allow_partial: bool = False,
    ...
    try:
        ...
        body += f"\n\n## 来源\n\n[小红书原笔记]({source['canonical_url']})\n"
        attrs = {"title": meta["title"].strip(), "category": "小红书图文笔记" if meta["media_type"] == "gallery" else "小红书视频笔记" if meta["media_type"] == "video" else "小红书混合笔记",
                 "url": source["canonical_url"], "origin": "小红书", "cover": relative_images[0],
        ...
        fd, temp_name = tempfile.mkstemp(prefix=".xhs-publish-", suffix=".tmp", dir=note_dir)
