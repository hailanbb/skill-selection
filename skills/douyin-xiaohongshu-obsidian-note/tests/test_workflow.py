...
from sources import canonical_source, classify, extract
...
class SourceTests(unittest.TestCase):
    ...
    def test_short_links_remain_unresolved(self):
        for host, platform in (("xhslink.com", "xiaohongshu"), ("xhslink.cn", "xiaohongshu"),
                               ("v.douyin.com", "douyin")):
            result = classify(f"http://{host}/a/Example")
            self.assertEqual(result["kind"], "short")
            self.assertEqual(result["platform"], platform)

    def test_douyin_video_and_profile_routes(self):
        source = "https://www.douyin.com/video/7531234567890123456?previous_page=app_code_link"
        result = canonical_source(source)
        self.assertEqual(result["platform"], "douyin")
        self.assertEqual(result["canonical_url"], "https://www.douyin.com/video/7531234567890123456")
        self.assertEqual(classify("https://www.douyin.com/user/MS4wLjABAAAAexample")["kind"], "profile")
        shared = classify("https://www.iesdouyin.com/share/video/7531234567890123456/?region=CN")
        self.assertEqual(shared["canonical_url"], result["canonical_url"])
    ...
    def test_asset_url_must_match_platform(self):
        with patch("socket.getaddrinfo", return_value=[(2, 1, 6, "", ("8.8.8.8", 443))]):
            with self.assertRaises(ValueError):
                validate_url("https://video.douyinvod.com/a.mp4", "xiaohongshu")
            self.assertEqual(validate_url("https://video.douyinvod.com/a.mp4", "douyin"),
                             "https://video.douyinvod.com/a.mp4")
...
class WorkflowTests(unittest.TestCase):
    ...
    def test_gallery_end_to_end_unicode_dimensions_and_links(self):
        ...
        self.assertFalse((self.vault / ".douyin-xiaohongshu-obsidian-note.publish.lock").exists())
    ...
    def test_same_source_id_on_two_platforms_is_not_a_duplicate(self):
        shared_id = "123456789012345678901234"
        self.meta["source_url"] = f"https://www.xiaohongshu.com/explore/{shared_id}"
        first = self.save()
        self.meta.update(source_url=f"https://www.douyin.com/video/{shared_id}", media_type="video",
                         content_basis="asr_and_frames", video_reviewed_to_end=True,
                         video_duration_seconds=30)
        self.meta["assets"][0].update(role="frame", timestamp_seconds=2)
        second = self.save()
    ...
    def test_douyin_video_uses_shared_pipeline_and_platform_attributes(self):
        self.meta.update(source_url="https://www.douyin.com/video/7531234567890123456?from=share",
                         media_type="video", content_basis="asr_and_frames",
                         video_reviewed_to_end=True, video_duration_seconds=60)
        self.meta["assets"][0].update(role="frame", timestamp_seconds=12.5)
        result = self.save(frame_width=1280)
        text = Path(result["note"]).read_text(encoding="utf-8")
        attrs, _ = split_frontmatter(text)
        self.assertEqual(attrs["origin"], "抖音")
        self.assertEqual(attrs["category"], "抖音视频笔记")
        self.assertEqual(attrs["url"], "https://www.douyin.com/video/7531234567890123456")
        self.assertIn("抖音原视频", text)

    def test_douyin_rejects_gallery_type(self):
        self.meta["source_url"] = "https://www.douyin.com/video/7531234567890123456"
        with self.assertRaisesRegex(ValueError, "Douyin video"):
            self.save()
    ...
    def test_download_mocked_response_creates_media_without_overwrite(self):
        ...
        write_json(request, {"url": "https://img.xhscdn.com/observed", "platform": "xiaohongshu", "kind": "image", "filename": "下载图.jpg"})
        response = MagicMock()
        ...
        with patch("fetch_asset.validate_url", side_effect=lambda value, platform=None: value), patch("fetch_asset.build_opener", return_value=opener):
    ...
    def test_video_tools_reject_outputs_or_inputs_outside_workspace(self):
        original = self.root / "用户原视频.mp4"
        original.write_bytes(b"not-a-real-video")
        frames = self.run / "frames"
        result = subprocess.run([sys.executable, "-X", "utf8", str(SCRIPTS / "extract_keyframes.py"),
                                 "--run-dir", str(self.run), "--video", str(original),
                                 "--out-dir", str(frames)], capture_output=True, encoding="utf-8")
        self.assertNotEqual(result.returncode, 0)
        self.assertFalse(frames.exists())
        outside_metadata = self.root / "metadata.json"
        result = subprocess.run([sys.executable, "-X", "utf8", str(SCRIPTS / "extract_metadata.py"),
                                 "--run-dir", str(self.run), "--url",
                                 "https://www.douyin.com/video/7531234567890123456",
                                 "--output", str(outside_metadata)], capture_output=True, encoding="utf-8")
        self.assertNotEqual(result.returncode, 0)
    ...
    def test_existing_lock_is_not_removed(self):
        lock = self.vault / ".douyin-xiaohongshu-obsidian-note.publish.lock"
