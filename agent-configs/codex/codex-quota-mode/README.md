...
**OpenAI Codex 专用 Skill**，属于「Agent 配置与行为规范」。v1.1 开启采用 **GPT-6 Astra / medium** 统筹、**GPT-5.6 Luna / medium** 按需执行；关闭恢复开启前的设置。它不承诺固定节省比例。
...
从已开启的 v1.0 升级：先用旧版执行 `status`，确认无冲突且没有 `pending`；再执行旧版 `off` 恢复开启前基线；备份并替换整个 Skill；如果需要继续保持开启，再用新版执行 `on`。不要直接替换脚本，避免旧版 high 配置与新版默认值冲突。
...
| 口令 | 行为 |
| --- | --- |
| 开启省额度模式 | 保存当前基线，写入 Astra/medium、Luna/medium、子线程上限 2，并追加全局调度规则 |
...
- **全局默认不等于当前窗口即时切换。** 新会话或重启后加载；当前受支持的界面可临时调整会话模型或推理强度，项目、profile、命令行和平台设置也可能覆盖全局默认。脚本不强行切换已运行的模型，也不自动升降强度或热切换当前轮。
...
- 不改变审批、沙箱或 MCP 设置，不重置或购买额度。medium 推理和多 Agent 协调仍有成本，订阅配额不能按 API 价格直接换算。
...
v1.1 已通过隔离目录中的 14 项开关测试和 1 项输出编码检查；Luna 独立前向测试是 v1.0 的历史验证记录。测试不写真实全局配置：
